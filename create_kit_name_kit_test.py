...
#Prueba 1. El parametro 'name' contiene un caracter / response code debe ser 201
def test_create_user_kit_1_letter_in_name_get_success_response():
    positive_assert("a")

#Prueba 2. El parametro 'name' contiene 511 caracteres / response code deber ser 201
def test_create_user_kit_511_letter_in_name_get_success_response():
    positive_assert(data.kit_body_test_2)

#Prueba 3. El parametro 'name' contiene cero caracteres / response code deber ser 400
def test_create_user_kit_empty_name_get_error_response():
    negative_assert ("")

#Prueba 4. El parametro 'name' contiene 512 characteres / response code deber ser 400
def test_create_user_kit_512_letter_in_name_get_error_response():
    negative_assert (data.kit_body_test_4)

#Prueba 5. El parametro 'name' permite caracteres especiales / response code deber ser 201
def test_create_user_kit_has_special_symbol_in_name_get_success_response():
    positive_assert("\"№%@\",")

#Prueba 6. El parametro 'name' permite espacios / response code debe ser 201
def test_create_user_kit_has_space_in_name_get_success_response():
    positive_assert("A Aaa")

#Prueba 7.El parametro 'name' permite números / response code deber ser 201
def test_create_user_kit_has_numbers_in_name_get_success_response():
    positive_assert("123")

#Prueba 8. La solicitud no contiene el parametro 'name' / response code debe ser 400
def test_create_user_kit_no_name_get_error_response():
    kit_body = data.kit_body.copy()
    kit_body.pop("name")
    negative_assert_no_name(kit_body)

#Prueba 9.El parametro 'name' contiene un tipo de dato diferente (int) / response code debe ser 400
